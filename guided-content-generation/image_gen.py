...
styles = ['Photorealistic', 'Comic', 'Pop-art',
          'Anime', 'Manga', 'Cyberpunk', 'Steampunk']
...
with st.sidebar:
    ...
    brand_protection = st.radio("Protect brands/products:", [
                                "off", "neutralize", "replace"], horizontal=True, help=brand_protection_help)
    brands = st.text_input('Brands to protect:', 'Microsoft, XBox',
                           help="Enter brand and product names that you want to enforce.") if brand_protection != "off" else None
    imgen_deployment = st.selectbox(
        "Image generation model:", ss.imgen_models, index=0, help="Image models with specified API key.")
    col1, col2 = st.columns(2)

    if imgen_deployment == 'FLUX.1 [pro]':
        flux_pro_aspect_ratios = ["1:1", "16:9",
                                  "2:3", "3:2", "4:5", "5:4", "9:16"]
        flux_pro_aspect_ratio = col1.selectbox(
            "Aspect Ratio", flux_pro_aspect_ratios, index=0)
        flux_pro_n_steps = col2.number_input(
            'Steps', min_value=1, max_value=50, value=25, format="%d", help="Number of diffusion steps between 1 and 50.")
        flux_pro_guidance = col1.number_input('Guidance', min_value=2, max_value=5, value=3, format="%d",
                                              help="Controls the balance between adherence to the text prompt and image quality/diversity between 2 and 5. Higher values make the output more closely match the prompt but may reduce overall image quality.")
        flux_pro_interval = col2.number_input('Interval', min_value=1, max_value=4, value=2, format="%d",
                                              help="Setting that increases the variance in possible outputs, letting the model be more dynamic in outputs.")
        ...
        sdxl_steps = col1.number_input('Steps', min_value=30, max_value=100, value=50,
                                       format="%d", help="Number of diffusion steps between 30 and 100.")
        ...
        dalle_size = st.selectbox(
            "Image size:", dalle_sizes, index=1, help="Image resolution width x height")

    elif imgen_deployment == "Stable Diffusion 3":
        sd3_aspect_ratios = ["1:1", "16:9", "21:9",
                             "2:3", "3:2", "4:5", "5:4", "9:16", "9:21"]
        sd3_models = ["sd3-medium", "sd3-large",
                      "sd3-large-turbo", "sd3.5-large-turbo", "sd3.5-large"]
        sd3_aspect_ratio = col2.selectbox(
            "Aspect Ratio", sd3_aspect_ratios, index=0)
    ...
    azure_image_analysis = bd_col2.checkbox(
        'AI Vision', False, disabled=not ss.azure_ai_vision, help='Use a custom object detection model trained on your data.')

    if azure_image_analysis:
        threshold = st.slider('Azure Image Analysis Threshold', 0.0, 1.0, 0.6, 0.1,
                              help='Lower values increase the likelihood of detecting objects but also increases false positives.')
    ...
    hate_thresh = ss.severity_to_id[col1.select_slider(
        "Hate", ss.severity_to_id.keys(), 'low')]
    selfharm_thresh = ss.severity_to_id[col2.select_slider(
        "SelfHarm", ss.severity_to_id.keys(), 'low')]
    sexual_thresh = ss.severity_to_id[col1.select_slider(
        "Sexual", ss.severity_to_id.keys(), 'low')]
    violence_thresh = ss.severity_to_id[col2.select_slider(
        "Violence", ss.severity_to_id.keys(), 'low')]

    save, cache = st.columns(2)
    save_images = save.checkbox("Save images", True)
    streamlit_cache = cache.checkbox(
        "Image cache", False, help="Applies to image prompts and generations. Video analysis results are always cached.")
...
def img2video(image_dir, filename):
    ...
    with st.spinner('Transforming image into video clip ...'):
        ...
        video_creator.resize_image_to_allowed_resolutions(
            video_target_folder, resized_filename)
        video_path = video_creator.image_to_video(
            video_target_folder, resized_filename, ss.stability_api_key)
...
if user_prompt:
    if brand_protection == "off":
        system_message = basic_system_message.format(
            style=imgen_style, model=imgen_deployment)
    elif brand_protection == "neutralize":
        system_message = neutralize_competitors_system_message.format(
            style=imgen_style, model=imgen_deployment, brands=brands)
    elif brand_protection == "replace":
        system_message = replace_competitors_system_message.format(
            style=imgen_style, model=imgen_deployment, brands=brands)
    ...
    def refine_user_prompt(prompt, model, system_message, cache=0.0):
        ...
        response = ss.aoai_client.chat.completions.create(
    ...
    with st.spinner('Validating and refining user prompt ...'):
        ...
        try:
            response = refine_user_prompt(
                user_prompt, ss.prompt_moderation_deployment, system_message, cache)
            ...
            st.markdown(
                ":blue[**Prompt ok**: " + prompt_filter_markdown + "]", unsafe_allow_html=True)
            refined_prompt = response.choices[0].message.content
        except Exception as e:
            if hasattr(e, 'code') and e.code == 'content_filter':
                prompt_filter = e.body['innererror']['content_filter_result']
                prompt_filter_markdown = display_moderation_results(
                    prompt_filter)
                st.markdown("**Prompt filtered**: " +
                            prompt_filter_markdown, unsafe_allow_html=True)
    ...
    def create_image(prompt, cache=0.0):
        ...
        if imgen_deployment == "FLUX.1 [pro]":
            ...
            response = create_sdxl_image(
                input_data, ss.aml_imgen_online_endpoint_url, ss.aml_imgen_api_key, ss.aml_deployment_name)
            ...
            response = requests.post(
                ...
                headers={
                    "authorization": f"Bearer {ss.stability_api_key}",
                    "accept": "image/*"
                },
                files={
                    "prompt": (None, prompt),
        ...
        if generated_image:
            ...
            if save_images:
                ...
                save_image_path = os.path.join(
                    image_dir, f'image_{max_id + 1:04}.png')
    ...
    with open(current_image_path, "rb") as file:
        download_col.download_button(label="Download image", data=file,
                                     file_name="generated_image.png", mime="image/png", use_container_width=True)

        if "Stable Diffusion 3" in ss.imgen_models:
            img2video_col.button(label="Generate video clip", key="img2video", on_click=img2video, args=(
                image_dir, 'generated_image.png',), use_container_width=True)
    ...
    with st.spinner('Check image for brands ...'):
        if azure_image_analysis:
            ...
            response_json = azure_image_analysis_predict(
                current_image_path, ss.vision_model_name, ss.vision_endpoint, ss.vision_key)
            duration = time.time() - start_time
            brands = set(object['tags'][0]['name'] for object in response_json['customModelResult']
                         ['objectsResult']['values'] if object['tags'][0]['confidence'] > threshold)
            brands_string = "Azure AI Vision Analysis: " + \
                ("Found " + ", ".join(brands) if brands else "No brands found")
    ...
    with st.spinner('Check image for harmful content ...'):
        ...
        content_safety_image_path = check_and_reduce_image_size(
            current_image_path)
        ...
        severity_thresholds = {'Hate': hate_thresh, 'SelfHarm': selfharm_thresh,
                               'Sexual': sexual_thresh, 'Violence': violence_thresh}
        ...
        st.markdown("**Image content safety**: " +
                    cs_results_markdown, unsafe_allow_html=True)
