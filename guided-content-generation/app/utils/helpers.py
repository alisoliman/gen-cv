...
async def analyze_image_gpt4o(image_path: str, prompt: str) -> Dict:
    ...
    # Use the exact system message from the original implementation
    system_message = """
...
"""
    ...
    try:
        ...
        # Create the completion - run in a thread to avoid blocking
        import asyncio
        ...
        return {
            "analysis": analysis,
            "brands": brands,
            "raw_response": str(response)  # Convert to string as the response object might not be JSON serializable
        }
    except Exception as e:
