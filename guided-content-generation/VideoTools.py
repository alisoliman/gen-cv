...
def download_youtube_video(url, target_dir="videos", max_retries=2):
    ...
    try:
        ...
        print(
            f"Unable to download video {title} after {max_retries} retries: {e}")
...
class VideoCreator:
        ...
        try:
            with Image.open(image_path) as img:
                ...
                save_path = os.path.join(
                    gen_folder, save_as) if save_as else image_path
                img.save(save_path)
    ...
    def image_to_video(gen_folder, source_image, stability_ai_key):
        ...
        if response.status_code != 200:
            raise Exception(
                f"Failed to start video generation: {response.text}")

        generation_id = response.json().get('id')
        if not generation_id:
            raise Exception("No generation ID received")

        target_image_path = VideoCreator.get_next_filename(
            gen_folder, source_image)
        while True:
            ...
            if response.status_code == 200:
                ...
                raise Exception(
                    f"Error during video generation: {response.text}")
    ...
    def concatenate_videos(gen_folder, video_files, output_file):
        ...
        try:
            ...
            final_clip.write_videofile(
                target_path, codec='libx264', audio_codec='aac')
...
class VideoExtractor:
    ...
    def transcribe_video(self, uri: str, openai_client, model) -> str:
        ...
        if clip.audio is not None:
            ...
            clip.audio.close()
            print(
                f"Extracted audio to {audio_path}. Transcription in progress ...")
    ...
    def extract_video_frames(self, interval: float) -> List[Dict[str, str]]:
        ...
        for frame_index in frame_indices:
            ...
            new_frame = np.zeros(
                (new_frame_height, frame.shape[1], 3), dtype=np.uint8)
            ...
            draw.rectangle(
                [(0, frame.shape[0]), (frame.shape[1], new_frame_height)], fill=(0, 0, 0))
        ...
        for scene in scene_list:
            ...
            for i in range(frames_per_scene):
                frame_index = start_frame + \
                    int((i + 1) / (frames_per_scene + 1) * scene_length)
                self.cap.set(cv2.CAP_PROP_POS_FRAMES, frame_index)
                ret, frame = self.cap.read()
                if not ret:
                    continue
...
class VideoAnalyzer:
    ...
    def video_chat(self, base64frames, transcription=None, system_message=None, max_retries=3, retry_delay=2):
        ...
        if system_message is None:
            ...
            {{
                ...
                "Outro": ["Start and end timestamp of the ending credits that show up in an episode. Empty list if none"],
        ...
        for attempt in range(max_retries):
            ...
            for i in range(0, len(base64frames), 50):
                    ...
                    messages=[
                        ...
                        {"role": "user", "content": [
                            *map(lambda x: {"type": "image_url", "image_url": {
                                 "url": f'data:image/jpg;base64,{x}', "detail": "auto"}}, frames_batch),
                ...
                try:
                    batch_response_dict = json.loads(response.choices[0].message.content)
        ...
        raise RuntimeError(
            "Failed to obtain valid response from the model after retries")
    ...
    def content_safety_moderate_video(self, video_frames: list, severity_thresholds=None):
        if severity_thresholds is None:
            severity_thresholds = {'Hate': 2,
                                   'SelfHarm': 2, 'Sexual': 2, 'Violence': 2}
        ...
        for video_frame in video_frames:
            moderation_results = self.content_safety_moderate_image(
            ...
            for check in moderation_results['categoriesAnalysis']:
                ...
                if check['severity'] >= severity_thresholds[check['category']]:
                    print(
                        f"Severity at frame {timestamp} for category {check['category']} is {detected_risk_label} and would be blocked at given threshold of {threshold_label}")
        ...
        def process_frame(video_frame):
            ...
            for check in moderation_results['categoriesAnalysis']:
                if check['severity'] >= severity_thresholds[check['category']]:
                    ...
                    frame_violations[check['category']].append(
    ...
    def video_chat_questions(self, base64frames, questions, transcription=None, system_message=None, max_retries=1, retry_delay=2):  # 3
        ...
        if system_message is None:
            system_message = f"""
                ...
                """
        question_messages = [{"type": "text", "text": question}
